...
public class EndpointJerseyTest extends JerseyTest {
	...
	public void testGetRevisionGraph(){
		...
		result = target("revisiongraph").queryParam("format", "table").queryParam("graph", ds1.graphName).request().get(String.class);
		Assert.assertThat(result, containsString("svg"));
		result = target("revisiongraph").queryParam("format", "graph").queryParam("graph", ds1.graphName).request().get(String.class);
		Assert.assertThat(result, containsString("visualisation"));
		result = target("revisiongraph").queryParam("format", "table").queryParam("graph", dsm.graphName).request().get(String.class);
		Assert.assertThat(result, containsString("svg"));
		result = target("revisiongraph").queryParam("format", "graph").queryParam("graph", dsm.graphName).request().get(String.class);
		Assert.assertThat(result, containsString("visualisation"));
	}
