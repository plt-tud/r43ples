			...
			try {
				...
				while (itPatternElts.hasNext()) {
					...
					if (!expression_list_revision_path.isEmpty()) {
						...
						ebp.addTriplePath(new TriplePath(var_rm1_old, new P_Link(RDF.type.asNode()),
								rmo_Revision));
						ebp.addTriplePath(new TriplePath(var_rm1_old, new P_Link(rmo_deltaRemoved),
								var_gm1_old));
						ebp.addTriplePath(new TriplePath(var_rm1, new P_OneOrMore1(new P_Link(
								prov_wasDerivedFrom)), var_rm1_old));
						eg_innerminus.addElement(ebp);

						eg_innerminus.addElementFilter(new ElementFilter(new E_OneOf(
								new ExprVar(var_rm1_old), expression_list_revision_path)));
