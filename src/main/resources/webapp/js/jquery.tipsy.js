...
(function ($) {
    ...
    Tipsy.prototype = {
        show: function () {
            ...
            if (title && this.enabled) {
                ...
                // Änderung auf 'wn' und 'ws' gravities. Ausrichtung nach rechts, Pfeil an der oberen oder unteren Ecke
                if (gravity.length == 2) {
                    if (gravity.charAt(1) == 'n') {
    ...
    // Änderung auf neue gravities 'wn' und 'ws', wenn der Tooltip zu dicht am unteren oder oberen Rand ist. Standardmäßig 'w'
    $.fn.tipsy.autoNS = function () {
        console.log($(this).offset());
        if($(this).offset().top > ($(document).scrollTop() + $(window).height() * 0.80)) return 'ws';
        if($(this).offset().top < ($(document).scrollTop() + $(window).height() * 0.20)) return 'wn';
        return 'w';
    };

    $.fn.tipsy.autoWE = function () {
        return $(this).offset().left > ($(document).scrollLeft() + $(window).width() / 3) ? 'e' : 'w';
    };
