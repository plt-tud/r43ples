...
public class CommitGraphView {
	...
	public CommitGraphView(List<Commit> commits) {
	...
	public void drawGraph(Graphics2D g) {
		...
		int currentLine = 0;
		int maxColumn = 0;
		g.translate(0, VisualisationTable.LINE_HEIGHT/2);
		...
		for (Commit c : commits) {
			...
			for (Commit suc : c.successors) {
				...
				int y = VisualisationTable.LINE_HEIGHT * currentLine + CIRCLE_DIAMETER / 2;
				path.moveTo(x, y);
		...
		dimension.setSize((maxColumn+1) * COLUMN_WIDTH, commits.size()
				* VisualisationTable.LINE_HEIGHT);
	...
	private void drawConnectionTo(GeneralPath path, int line,
		...
		int y2 = VisualisationTable.LINE_HEIGHT * line + CIRCLE_DIAMETER / 2;
		...
		if (x1 == x2) {
			...
			// draw curved line
			int curvedLineBase = (int) (VisualisationTable.LINE_HEIGHT * 0.7);
	...
	private void drawCircle(Graphics2D g, int line, int column) {
		g.fillOval(COLUMN_WIDTH * column, VisualisationTable.LINE_HEIGHT * line, CIRCLE_DIAMETER,
				CIRCLE_DIAMETER);
	}
