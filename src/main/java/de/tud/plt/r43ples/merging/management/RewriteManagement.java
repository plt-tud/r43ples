...
public class RewriteManagement {
	/** The logger. **/
	...
	public static String rewriteQuery(final String query_r43ples) throws InternalErrorException {
		...
		while (m.find()) {
			...
			/** in alter Version ist 
			 * LinkedList<Revision> list = RevisionManagement.getTree(graphName).getPathToRevision(revisionNumber);
			 * but here in new Version, there is no getTree(), but man can mit Tree(graphName) replace*/
